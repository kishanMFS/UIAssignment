...
async function recurseRight(impls, info, handler, index) {
  ...
  return {
    type: "error",
    value: /* @__PURE__ */ new Error(
      "No result assigned in instrumentation chain.",
    ),
  };
}
...
function createRouter(init) {
  ...
  function initialize() {
    ...
    if (!state.initialized)
      startNavigation("POP", state.location, { initialHydration: true });
    return router;
  }
  function dispose() {
  ...
  async function handleFetcherAction(
    ...
    updateFetcherState(
      key,
      getSubmittingFetcher(submission, state.fetchers.get(key)),
      { flushSync },
    );
    ...
    let fetcherResultsPromise = Promise.all(
      fetchersToLoad.map(async (f) => {
        if (f.matches && f.match && f.request && f.controller) {
          let result = (
            await callDataStrategy(f.request, f.matches, scopedContext, f.key)
          )[f.match.route.id];
          return { [f.key]: result };
        } else
    ...
    if (generateMiddlewareResponse) {
      ...
      try {
        ...
        let response = await runServerMiddlewarePipeline(
          ...
          async () => {
            return await generateMiddlewareResponse(
...
var loadLazyRouteProperty = ({
  ...
  let propertyPromise = (async () => {
    ...
    let isStaticallyDefined =
      routeToUpdate[key] !== void 0 && key !== "hasErrorBoundary";
    if (isUnsupported) {
      warning(
        !isUnsupported,
        "Route property " +
  ...
  if (typeof route.lazy === "function") {
    ...
    let lazyRoutePromise2 = (async () => {
      ...
      for (let lazyRouteProperty in lazyRoute) {
        ...
        let isStaticallyDefined =
          routeToUpdate[lazyRouteProperty] !== void 0 &&
...
async function defaultDataStrategy(args) {
  ...
  let keyedResults = {};
  (await Promise.all(matchesToLoad.map((m) => m.resolve()))).forEach(
    (result, i) => {
...
var ViewTransitionContext = import_react.createContext({
...
var AwaitContextProvider = (props) =>
  import_react.createElement(AwaitContext.Provider, props);
...
function useIsomorphicLayoutEffect(cb) {
  if (!import_react.useContext(NavigationContext).static)
    import_react.useLayoutEffect(cb);
}
...
var defaultErrorElement = /* @__PURE__ */ import_react.createElement(
  ...
  return renderedMatches.reduceRight((outlet, match, index) => {
    ...
    let getChildren = () => {
      ...
      if (error) children = errorElement;
        ...
        children = /* @__PURE__ */ import_react.createElement(
...
function RouterProvider({
  ...
  let [vtContext, setVtContext] = import_react.useState({
    isTransitioning: false,
  });
...
var objectProtoNames2 = Object.getOwnPropertyNames(Object.prototype)
  .sort()
  .join("\0");
        ...
        const racePromise = new Promise((resolve, reject) => {
          ...
          if (signal) {
            const rejectPromise = () =>
              reject(
                signal.reason ||
                  /* @__PURE__ */ new Error("Signal was aborted."),
              );
...
function stripIndexParam$1(url) {
  ...
  for (let indexValue of indexValues)
    if (indexValue) indexValuesToKeep.push(indexValue);
  ...
  return /* @__PURE__ */ import_react.createElement(
    ...
    /* @__PURE__ */ import_react.createElement(
      ...
      /* @__PURE__ */ import_react.createElement(
        ...
        children,
        renderScripts
          ? /* @__PURE__ */ import_react.createElement(Scripts, null)
          : null,
      ),
    ),
  );
...
var Link = import_react.forwardRef(function LinkWithRef(
  ...
  let { basename, navigator, unstable_useTransitions } =
    import_react.useContext(NavigationContext);
    ...
    (event) => {
      if (shouldProcessLinkClick(event, target)) {
        ...
        if (unstable_useTransitions)
          import_react.startTransition(() => doNavigate());
        else doNavigate();
      }
    },
    [
...
function useSearchParams(defaultInit) {
  ...
  let defaultSearchParamsRef = import_react.useRef(
...
var require_dist = /* @__PURE__ */ __commonJSMin((exports) => {
  ...
  function stringifySetCookie(_name, _val, _opts) {
    ...
    const enc =
      (typeof _val === "object" ? _val : _opts)?.encode || encodeURIComponent;
...
var unsign = async (cookie, secret) => {
  ...
  try {
    ...
    return (await crypto.subtle.verify("HMAC", key, signature, data2))
      ? value
      : false;
  } catch (error) {
    return false;
...
function prependCookies(parentHeaders, childHeaders) {
  ...
  if (parentSetCookieString) {
    let cookies = (0, import_set_cookie.splitCookiesString)(
...
function parseHostHeader(headers) {
  let forwardedHostValue = headers
...
function errorResponseToJson(errorResponse, serverMode) {
  return Response.json(
    serializeError(
      errorResponse.error ||
        /* @__PURE__ */ new Error("Unexpected Server Error"),
      serverMode,
    ),
    {
...
function HydratedRouter(props) {
  ...
  let [criticalCss, setCriticalCss] = import_react.useState(
